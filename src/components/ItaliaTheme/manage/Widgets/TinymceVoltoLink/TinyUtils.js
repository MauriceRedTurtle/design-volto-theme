...
const collectNodesInRange = <T extends Node>(
  rng: Range,
  predicate: (node) => node is T,
): T[] => {
  if (rng.collapsed) {
    return [];
  } else {
...
const hasLinksInSelection = (rng: Range) =>
...
const isOnlyTextSelected = (editor: Editor) => {
  ...
  const isElement = (elm: Node): elm is Element =>
    elm.nodeType === 1 &&
    !isAnchor(elm) &&
    !Obj.has(inlineTextElements, elm.nodeName.toLowerCase());
